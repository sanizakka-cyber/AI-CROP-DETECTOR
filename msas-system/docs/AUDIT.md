...
- **Rolled out to 20 of 22 dashboards** (184 `safe()` call sites total): `farmer`, `admin`, `vet`, `agronomist`, `dealer`, `hr`, `equipment-dealer`, `extension`, `finance`, `dataAnalyst`, `fieldOfficer`, `cooperative`, `ngo`, `government`, `researchInstitution`, `investor`, `financialInstitution`, `logistics`, `agribusiness`, `inputSupplier`. Every `try/catch` in these 20 controller methods converted to `safe()`, `$dashboardErrors` passed to each view, `<x-dashboard-error-banner>` wired in at the top of each page. Verified after each batch by brace/paren balance-checking the whole file (no PHP CLI available to lint directly) — consistently balanced across all edits.
- **3 dashboards deliberately not converted:** `operations()` (1 multi-statement block), `monitoringEvaluation()` (5 such blocks), `customerSupport()` (3 such blocks) — these compute several intermediate values inside one try block before a final result, which need regular closures with explicit `use()` captures rather than the simpler `fn() =>` form used above. Flagged separately rather than rushed, since getting a capture wrong here would silently change what a stat measures, not just how failures are reported.
- `rider()` delegates entirely to `RiderController::dashboard()` — out of scope for this controller-level pattern; would need its own conversion inside that controller if wanted.
- None of the untouched methods regressed — they're byte-for-byte the same as before this work started, still showing the original silent-0-on-failure behavior, just not yet improved.
...
| Error/loading/empty state handling | Infrastructure built (`safe()` helper + `<x-dashboard-error-banner>`), rolled out to **20/22 dashboards** (184 safe() call sites). 3 remaining (operations, monitoringEvaluation, customerSupport) need more careful closure conversion — multi-statement try blocks, deliberately not rushed (§11.4) |
