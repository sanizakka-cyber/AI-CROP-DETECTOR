...
- **Rolled out to 7 of 22 dashboards:** `farmer` (13 query sites, highest traffic), `admin` (24 sites), `vet`, `agronomist`, `dealer`, `hr`, `equipment-dealer`. Every `try/catch` in these 7 controller methods converted to `safe()`, `$dashboardErrors` passed to each view, `<x-dashboard-error-banner>` wired in at the top of each page.
- **15 dashboards remain**, in two groups:
  - **Straightforward (single-expression try/catch, same mechanical conversion as the 7 above):** finance, dataAnalyst, cooperative, ngo, government, researchInstitution, investor, financialInstitution, logistics, agribusiness, inputSupplier, fieldOfficer, extension — 13 methods.
  - **Genuinely more involved (multi-statement try blocks that compute several intermediate values before one final result — these need regular closures with explicit `use()` captures, not the simpler `fn() =>` form used above, to convert correctly without changing behavior):** `operations()` (1 such block), `monitoringEvaluation()` (5 such blocks), `customerSupport()` (3 such blocks). Flagged separately rather than rushed, since getting a capture wrong here would silently change what a stat measures, not just how failures are reported.
- None of the 15 untouched dashboards regressed — they're byte-for-byte the same as before this work started, still showing the original silent-0-on-failure behavior, just not yet improved.
...
| Error/loading/empty state handling | Infrastructure built (`safe()` helper + `<x-dashboard-error-banner>`), rolled out to **7/22 dashboards** (farmer, admin, vet, agronomist, dealer, hr, equipment-dealer). 13 remaining are mechanical repetition; 3 (operations, monitoringEvaluation, customerSupport) need more careful closure conversion (§11.4) |
