<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <a href="{{ route('equipment-dealer.products.index') }}" class="px-4 py-2 bg-slate-100 text-slate-700 rounded-xl text-sm font-semibold hover:bg-slate-200 transition self-start">
                ← Back to Products
            </a>
        </div>
        ...
        <form method="GET" class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 flex gap-3 items-end">
            ...
            @if(request('status'))<a href="{{ route('equipment-dealer.orders') }}" class="px-4 py-2 bg-slate-100 text-slate-600 rounded-xl text-sm font-semibold hover:bg-slate-200 transition">Clear</a>@endif
        </form>
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
                ...
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <form method="POST" action="{{ route('equipment-dealer.orders.status', $order) }}">
                        @csrf @method('PATCH')
                        <div class="flex items-center gap-2">
                            <select name="status" class="border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0F6B3E]/30">
