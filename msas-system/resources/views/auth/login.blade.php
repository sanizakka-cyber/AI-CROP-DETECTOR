<x-guest-layout>
    ...
    <div class="fade-up-2" style="margin-bottom:24px;">
        <h2 style="font-family:'Poppins',sans-serif;font-size:22px;font-weight:800;color:#0f172a;margin:0 0 5px 0;letter-spacing:-0.3px;">{{ __('Welcome back') }}</h2>
        <p style="font-size:13px;color:#64748b;margin:0;">{{ __('Sign in to your MSAS portal account') }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}" autocomplete="off" id="login-form">
        ...
        <div class="fade-up-3" style="margin-bottom:16px;">
            <label for="login" class="field-label">{{ __('Email or Phone Number') }}</label>
            <div style="position:relative;">
                ...
                <div id="login-suggestions"
                    ...
                    <div style="padding:7px 12px 5px;font-size:10px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:0.08em;border-bottom:1px solid #f1f5f9;display:flex;justify-content:space-between;align-items:center;">
                        <span>{{ __('Suggested account') }}</span>
                        <button type="button" onclick="clearAllAccounts(event)" style="background:none;border:none;color:#ef4444;font-size:10px;font-weight:700;cursor:pointer;padding:2px 6px;border-radius:4px;" onmouseover="this.style.background='#fef2f2'" onmouseout="this.style.background='none'">{{ __('Clear All') }}</button>
                    </div>
        ...
        <div class="fade-up-3" style="margin-bottom:16px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                <label for="password" class="field-label" style="margin-bottom:0;">{{ __('Password') }}</label>
                @if(Route::has('password.request'))
                    <a href="{{ route('password.request') }}" style="font-size:12px;color:#0F6B3E;text-decoration:none;font-weight:600;">{{ __('Forgot password?') }}</a>
                @endif
            </div>
            <div style="position:relative;">
        ...
        <div class="fade-up-4" style="margin-bottom:22px;">
            <label style="display:flex;align-items:center;gap:10px;cursor:pointer;user-select:none;">
                ...
                <span style="font-size:13px;color:#475569;font-weight:500;">{{ __('Keep me signed in') }}</span>
            </label>
        </div>
        ...
        <div class="fade-up-5" style="text-align:center;padding-top:16px;border-top:1px solid #f1f5f9;">
            <span style="font-size:13px;color:#94a3b8;">{{ __("Don't have an account?") }}</span>
            <a href="{{ route('register') }}" style="font-size:13px;color:#0F6B3E;font-weight:700;text-decoration:none;margin-left:5px;">{{ __('Create Account') }}</a>
        </div>
    </form>

    <script>
    var STORAGE_KEY = 'msas_remembered_accounts';
    var _t = { signingIn: @json(__('Signing in…')) };
    ...
    document.getElementById('login-form').addEventListener('submit', function(e) {
        ...
        btn.disabled = true;
        btn.innerHTML = '<span style="display:flex;align-items:center;justify-content:center;gap:8px;"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="animation:spin 1s linear infinite"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg> ' + _t.signingIn + '</span>';
    });
    </script>
