    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-5">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col group hover:shadow-md hover:border-emerald-200 transition-all duration-200">
            ...
            <div class="p-3 md:p-4 flex flex-col flex-1 gap-1">
                ...
                <button
                    onclick="openContactModal(@json(['name'=>$product->name,'price'=>'₦'.number_format($product->selling_price,2).($product->unit ? ' / '.$product->unit : ''),'dealer'=>$dealerName,'phone'=>$dealerPhone,'category'=>$product->category]))"
                    class="mt-2 w-full py-2 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white text-xs font-bold rounded-xl transition flex items-center justify-center gap-1.5 shadow-sm">
