<x-app-layout>
    <x-slot name="header">My Marketplace Orders</x-slot>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                ...
                <p class="text-slate-500 text-sm mt-0.5">Customer orders placed on your marketplace listings</p>
            </div>
            <a href="{{ route('marketplace.sell') }}" class="px-4 py-2 bg-slate-100 text-slate-700 rounded-xl text-sm font-semibold hover:bg-slate-200 transition self-start">
                ← Back to Listings
            </a>
        </div>
        ...
        <form method="GET" class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 flex gap-3 items-end">
            ...
            @if(request('status'))<a href="{{ route('marketplace.sell.orders') }}" class="px-4 py-2 bg-slate-100 text-slate-600 rounded-xl text-sm font-semibold hover:bg-slate-200 transition">Clear</a>@endif
        </form>
        ...
        <div class="space-y-4">
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-50">
                            ...
                            <tr>
                                ...
                                <td class="px-3 py-2 font-bold text-slate-800 text-right">₦{{ number_format($item->total_price) }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 py-16 text-center">
                ...
                <p class="text-slate-300 text-sm mt-1">Orders from buyers will appear here once you have active listings.</p>
            </div>
            @endforelse
        </div>
