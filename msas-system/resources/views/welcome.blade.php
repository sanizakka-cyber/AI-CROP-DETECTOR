...
<head>
    ...
    <style>
        ...
        .nav-link{position:relative;padding-bottom:2px;font-weight:500;font-size:.875rem;transition:color .2s;}
        ...
        .btn-primary{display:inline-flex;align-items:center;justify-content:center;gap:.5rem;padding:.6rem 1.25rem;background:var(--green);color:#fff;border-radius:8px;font-weight:600;font-size:.875rem;transition:all .2s;border:2px solid var(--green);}
        ...
        .section-title{font-size:clamp(1.6rem,3vw,2.2rem);font-weight:800;color:#111;line-height:1.25;margin-bottom:.75rem;}
        .section-sub{font-size:1rem;color:#555;max-width:600px;margin:0 auto;line-height:1.7;}

        .sol-card{background:#fff;border:1px solid #e5e7eb;border-radius:12px;padding:1rem .75rem;text-align:center;transition:all .3s;cursor:pointer;}
        .sol-card:hover{transform:translateY(-4px);box-shadow:0 8px 24px rgba(46,125,50,.15);border-color:var(--green);}
        .sol-icon{width:40px;height:40px;border-radius:10px;background:var(--green-light);display:flex;align-items:center;justify-content:center;font-size:1.1rem;color:var(--green);margin:0 auto .5rem;}

        .feat-card{background:#fff;border:1px solid #e5e7eb;border-radius:14px;padding:1.25rem;display:flex;gap:.875rem;align-items:flex-start;transition:all .3s;}
        .feat-card:hover{box-shadow:0 8px 24px rgba(0,0,0,.08);border-color:var(--green);}
        .feat-icon{width:40px;height:40px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:1rem;flex-shrink:0;}

        .price-card{background:#fff;border:2px solid #e5e7eb;border-radius:20px;padding:1.75rem;transition:all .3s;}
...
<header id="main-nav" class="fixed top-0 inset-x-0 z-50 transition-all" x-data="{ open:false, scrolled:false }"
    ...
    <div class="max-w-7xl mx-auto px-4 flex items-center justify-between h-16">
        <a href="/" class="flex items-center gap-2 shrink-0">
            <div class="w-9 h-9 rounded-full flex items-center justify-center" style="background:var(--green)"><i class="fa-solid fa-leaf text-white text-sm"></i></div>
        ...
        <button @click="open=!open" class="lg:hidden p-2 rounded-lg transition" :class="scrolled?'text-gray-800 hover:bg-gray-100':'text-white hover:bg-white/10'">
    ...
    <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="lg:hidden bg-white border-t border-gray-100 shadow-xl">
        <nav class="px-4 py-3 space-y-1 border-b border-gray-100">
            ...
            <a href="{{ $href }}" @click="open=false" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-700 hover:bg-green-50 hover:text-green-800 font-medium text-sm transition"><i class="fa-solid fa-{{ $icon }} w-4 text-green-600 text-sm"></i> {{ $label }}</a>
            @endforeach
        </nav>
...
<section id="home" class="hero-bg min-h-screen flex flex-col justify-center pt-16">
    <div class="max-w-7xl mx-auto px-4 py-20 grid lg:grid-cols-12 gap-10 items-center">
        {{-- Left: headline + CTAs --}}
        <div class="lg:col-span-8 text-white">
            ...
            <h1 class="font-heading text-4xl md:text-5xl lg:text-[56px] font-extrabold leading-[1.15] mb-5">
                Empowering Agriculture<br/>with <span class="text-yellow-300">Artificial Intelligence</span>
            </h1>
            <p class="text-green-100 text-lg leading-relaxed mb-8 max-w-xl">Helping farmers, livestock owners, agribusinesses, governments and development partners make smarter decisions through AI, data and digital innovation.</p>
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('register') }}" class="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl font-bold text-base shadow-lg transition hover:-translate-y-1" style="background:var(--green);color:#fff;box-shadow:0 8px 24px rgba(46,125,50,.45)"><i class="fa-solid fa-seedling"></i> Start Farming</a>
                <a href="#solutions" class="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl font-semibold text-base bg-white/15 backdrop-blur border border-white/30 text-white hover:bg-white/25 transition"><i class="fa-solid fa-play text-xs"></i> Watch Demo</a>
                <a href="{{ route('register') }}" class="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl font-semibold text-base bg-white/10 backdrop-blur border border-white/20 text-white hover:bg-white/20 transition"><i class="fa-solid fa-download text-xs"></i> Download App</a>
            </div>
        </div>
        ...
        <div class="lg:col-span-4 flex flex-col gap-3">
            ...
            <div class="flex items-center gap-4 bg-white rounded-xl px-5 py-3.5 shadow-lg">
...
<section class="py-10 bg-white border-b border-gray-100">
    <div class="max-w-6xl mx-auto px-4">
        <p class="text-center text-xs font-bold uppercase tracking-widest text-gray-400 mb-8">Trusted by Leading Organizations</p>
        <div class="flex flex-wrap items-center justify-center gap-8 lg:gap-14">
            ...
            <div class="flex flex-col items-center gap-2 group cursor-default">
                <i class="fa-solid fa-{{ $icon }} text-2xl text-gray-300 group-hover:text-green-600 transition"></i>
                <span class="font-bold text-[11px] text-gray-400 group-hover:text-green-700 transition text-center max-w-[80px] leading-tight">{{ $name }}</span>
            </div>
            @endforeach
            <div class="text-gray-300 font-semibold text-xs border border-gray-200 rounded-lg px-3 py-2 text-center leading-tight">And Many<br/>More...</div>
        </div>
    </div>
</section>
    ...
    <div class="max-w-7xl mx-auto px-4">
        <div class="grid lg:grid-cols-3 gap-10 items-start">
            ...
            <div class="grid grid-cols-2 gap-3">
                <img src="https://images.unsplash.com/photo-1586771107445-d3ca888129ff?w=300&q=80&auto=format&fit=crop" alt="Crop farming" class="rounded-xl object-cover h-40 w-full">
                <img src="https://images.unsplash.com/photo-1605000797499-95a51c5269ae?w=300&q=80&auto=format&fit=crop" alt="Farmers" class="rounded-xl object-cover h-40 w-full mt-6">
                <img src="https://images.unsplash.com/photo-1548550023-2bdb3c5beed7?w=300&q=80&auto=format&fit=crop" alt="Poultry" class="rounded-xl object-cover h-40 w-full -mt-6">
                <img src="https://images.unsplash.com/photo-1570042225831-d98fa7577f1e?w=300&q=80&auto=format&fit=crop" alt="Livestock" class="rounded-xl object-cover h-40 w-full">
            </div>
            {{-- Text --}}
            <div>
                <div class="section-tag"><i class="fa-solid fa-circle-info"></i> About MSAS Agro</div>
                <h2 class="font-heading font-extrabold text-2xl text-gray-900 mb-4">About <span style="color:var(--green)">MSAS Agro</span></h2>
                <p class="text-gray-500 text-sm leading-relaxed mb-5">MSAS Agro is an AI-powered digital platform that provides innovative solutions for crop farming, livestock management, poultry, fish farming, marketplace, finance, insurance, and data analytics. Since 2019, we have been driving agriculture transformation.</p>
            ...
            <div class="rounded-2xl overflow-hidden border border-gray-100 shadow-lg">
                <div class="relative">
                    <img src="https://images.unsplash.com/photo-1574943320219-553eb213f72d?w=500&q=80&auto=format&fit=crop" alt="Agriculture Technology" class="w-full h-52 object-cover">
    ...
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-8 gap-3">
            @foreach([
                ...
                ['Government Dashboard','landmark','register'],
                ['NGO Dashboard','hand-holding-heart','register'],
                ['Research Portal','microscope','register'],
                ['Data Analytics','chart-bar','register'],
                ['Training & E-Learning','graduation-cap','register'],
                ['Cooperative Management','people-group','register'],
            ...
            <a href="{{ route($route) }}" class="sol-card fade-up group">
                ...
                <div class="font-semibold text-gray-700 text-[11px] leading-tight group-hover:text-green-700 transition">{{ $name }}</div>
            </a>
            @endforeach
            <a href="{{ route('register') }}" class="sol-card fade-up flex flex-col items-center justify-center" style="border-color:var(--green);background:var(--green-light)">
                <div class="sol-icon mb-1" style="background:var(--green);color:#fff"><i class="fa-solid fa-ellipsis"></i></div>
                <div class="font-bold text-xs" style="color:var(--green)">And More</div>
            </a>
        </div>
...
<section class="py-24 bg-white">
    <div class="max-w-7xl mx-auto px-4 grid lg:grid-cols-2 gap-16 items-center">
        <div>
            ...
            <h2 class="font-heading font-extrabold text-3xl text-gray-900 mb-4">Meet Your <span style="color:var(--green)">AI Farm Assistant</span></h2>
            <p class="text-gray-500 leading-relaxed mb-6 text-sm">Get instant answers, smart recommendations and real-time insights to improve your productivity. Powered by advanced vision AI that diagnoses crop diseases, livestock conditions, and soil health from a single photo.</p>
        ...
        <div class="bg-gray-50 rounded-2xl border border-gray-200 overflow-hidden shadow-lg">
            <div class="flex items-center justify-between px-5 py-3.5 border-b border-gray-200 bg-white">
                <div class="flex items-center gap-2.5">
                    <div class="w-9 h-9 rounded-full flex items-center justify-center text-white text-sm font-bold" style="background:var(--green)"><i class="fa-solid fa-robot text-xs"></i></div>
            ...
            <div class="p-5 space-y-4 min-h-[180px]">
                <div class="flex gap-3"><div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs shrink-0" style="background:var(--green)"><i class="fa-solid fa-robot text-[10px]"></i></div><div class="bg-white rounded-2xl rounded-tl-none px-4 py-2.5 shadow-sm border border-gray-100 text-xs text-gray-700 max-w-xs">Hello! I am your AI Farm Assistant. How can I help you today?</div></div>
                <div class="flex justify-end"><div class="rounded-2xl rounded-tr-none px-4 py-2.5 text-xs text-white max-w-xs" style="background:var(--green)">What is the best fertilizer for maize at vegetative stage?</div></div>
                <div class="flex gap-3"><div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs shrink-0" style="background:var(--green)"><i class="fa-solid fa-robot text-[10px]"></i></div><div class="bg-white rounded-2xl rounded-tl-none px-4 py-2.5 shadow-sm border border-gray-100 text-xs text-gray-700 max-w-xs">For maize at vegetative stage, <strong>NPK 20:10:10</strong> at 100kg/ha is recommended. Also ensure good weed control and adequate irrigation.</div></div>
            </div>
...
<section class="py-24 bg-gray-50">
    <div class="max-w-5xl mx-auto px-4">
        ...
        <div class="flex flex-col md:flex-row items-start justify-center gap-0">
            @php
            $steps = [
            ...
            <div class="flex items-start md:flex-1">
                <div class="text-center flex-1 fade-up px-2">
                ...
                @if($idx < 3)
                <div class="hidden md:flex items-center pt-5 shrink-0">
                    <i class="fa-solid fa-arrow-right text-lg" style="color:var(--green);opacity:.5"></i>
                </div>
                @endif
            </div>
            @endforeach
...
{{-- ═══════════════════ MOBILE APP ═══════════════════ --}}
<section class="py-24 text-white relative overflow-hidden" style="background:linear-gradient(135deg,#1B5E20 0%,#2E7D32 55%,#0288D1 100%)">
    ...
    <div class="max-w-7xl mx-auto px-4 relative z-10">
        <div class="grid lg:grid-cols-3 gap-12 items-center">
            <div>
                <h2 class="font-heading font-extrabold text-3xl mb-3">Take MSAS Agro<br/><span class="text-yellow-300">Anywhere You Go</span></h2>
                <p class="text-green-100 text-sm leading-relaxed mb-6">Our mobile app is fast, offline-ready and designed for farmers. Record data in the field, get AI diagnoses, check market prices — even without internet.</p>
                ...
                <div class="flex items-center gap-3">
                    <div class="w-16 h-16 bg-white rounded-xl p-2 shrink-0 flex items-center justify-center"><i class="fa-solid fa-qrcode text-gray-800 text-3xl"></i></div>
            ...
            <div class="flex justify-center gap-3 items-end">
                <div class="w-28 h-52 bg-white/15 backdrop-blur border border-white/25 rounded-2xl overflow-hidden shadow-2xl">
                    ...
                    <div class="p-2 flex flex-col gap-1.5">
                        ...
                        <div class="grid grid-cols-2 gap-1"><div class="bg-white/20 rounded h-8"></div><div class="bg-white/20 rounded h-8"></div><div class="bg-white/20 rounded h-8"></div><div class="bg-white/20 rounded h-8"></div></div>
                    </div>
                </div>
                <div class="w-28 h-60 bg-white/20 backdrop-blur border border-white/30 rounded-2xl overflow-hidden shadow-2xl">
                    <div class="h-2.5 bg-white/20 flex items-center px-2"><span class="w-1.5 h-1.5 rounded-full bg-white/40"></span></div>
                    <div class="p-2 flex flex-col gap-1.5">
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center gap-3 bg-white/10 border border-white/15 rounded-xl px-4 py-3">
                    <div class="w-6 h-6 rounded-full flex items-center justify-center shrink-0" style="background:var(--green)"><i class="fa-solid fa-check text-white text-[9px]"></i></div>
                    <span class="text-sm font-medium text-green-50">{{ $feat }}</span>
                </div>
                @endforeach
            </div>
        </div>
...
<section class="py-14 bg-white border-b border-gray-100">
    <div class="max-w-6xl mx-auto px-4">
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6 text-center">
            @foreach([['20,000+','Registered Farmers','users'],['150+','Cooperatives','handshake'],['100+','Development Projects','folder'],['500+','Extension Agents','person-chalkboard'],['36 States','Nationwide Coverage','map'],['99.9%','System Uptime','server']] as [$num,$label,$ico])
            <div class="fade-up">
                <i class="fa-solid fa-{{ $ico }} text-2xl mb-2 block" style="color:var(--green)"></i>
                <div class="font-heading font-extrabold text-2xl text-gray-900 leading-none">{{ $num }}</div>
                <div class="text-gray-400 text-xs font-medium mt-1">{{ $label }}</div>
            </div>
            @endforeach
        </div>
    </div>
...
<section id="testimonials" class="py-24 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="relative">
            ...
            <button class="absolute -left-5 top-1/2 -translate-y-1/2 w-10 h-10 bg-white rounded-full shadow-lg border border-gray-100 items-center justify-center hover:bg-gray-50 transition hidden md:flex"><i class="fa-solid fa-chevron-left text-gray-600 text-sm"></i></button>
            <button class="absolute -right-5 top-1/2 -translate-y-1/2 w-10 h-10 bg-white rounded-full shadow-lg border border-gray-100 items-center justify-center hover:bg-gray-50 transition hidden md:flex"><i class="fa-solid fa-chevron-right text-gray-600 text-sm"></i></button>
        </div>
    </div>
</section>

{{-- ═══════════════════ MARKETPLACE ═══════════════════ --}}
<section id="marketplace" class="py-24 bg-white">
    <div class="max-w-7xl mx-auto px-4 grid lg:grid-cols-2 gap-16 items-center">
        <div>
            ...
            <div class="grid grid-cols-2 gap-3 mb-7">
                ...
                <div class="flex items-center gap-3 bg-gray-50 rounded-xl p-3 border border-gray-100">
                    <div class="w-9 h-9 rounded-lg bg-{{ $c }}-100 flex items-center justify-center shrink-0"><i class="fa-solid fa-{{ $i }} text-{{ $c }}-600 text-sm"></i></div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div class="bg-gray-50 rounded-xl p-5 border border-gray-100 text-center"><div class="text-2xl font-extrabold mb-1" style="color:var(--green)">120+</div><div class="text-gray-500 text-xs">Active Listings</div></div>
            <div class="bg-gray-50 rounded-xl p-5 border border-gray-100 text-center"><div class="text-2xl font-extrabold text-yellow-500 mb-1">₦0</div><div class="text-gray-500 text-xs">Listing Fee</div></div>
            <div class="bg-gray-50 rounded-xl p-5 border border-gray-100 text-center"><div class="text-2xl font-extrabold mb-1" style="color:var(--blue)">35+</div><div class="text-gray-500 text-xs">Verified Dealers</div></div>
            <div class="bg-gray-50 rounded-xl p-5 border border-gray-100 text-center"><div class="text-2xl font-extrabold text-purple-600 mb-1">NGN</div><div class="text-gray-500 text-xs">Naira Payments</div></div>
        </div>
    </div>
</section>
        ...
        <div class="grid lg:grid-cols-3 gap-8">
            <div class="space-y-5">
                ...
                <div class="flex gap-4 bg-gray-50 rounded-2xl p-5 border border-gray-100">
                    ...
                    <div><div class="font-bold text-gray-800 text-sm mb-0.5">{{ $label }}</div><div class="text-gray-500 text-sm break-all">{{ $val }}</div></div>
                </div>
                @endforeach
                <div class="flex gap-3">
                    <a href="https://wa.me/2348129582957" class="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-white font-semibold text-sm hover:opacity-90 transition" style="background:#25D366"><i class="fa-brands fa-whatsapp"></i> WhatsApp</a>
                    <a href="tel:08129582957" class="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-sm border-2 hover:opacity-90 transition" style="border-color:var(--green);color:var(--green)"><i class="fa-solid fa-phone"></i> Call Now</a>
                </div>
...
{{-- ═══════════════════ FOOTER ═══════════════════ --}}
<footer style="background:var(--footer-bg)" class="text-gray-400 pt-16 pb-6">
    <div class="max-w-7xl mx-auto px-4">
        <div class="grid md:grid-cols-6 gap-8 mb-10">
            {{-- Brand col --}}
            <div class="md:col-span-2">
                ...
                <div class="text-xs font-semibold mb-4" style="color:var(--green)">Smart Agriculture, Better Tomorrow</div>
            ...
            <div>
                ...
                <ul class="space-y-2.5 text-sm">
                    ...
                    <li><a href="{{ $href }}" class="hover:text-green-400 transition">{{ $label }}</a></li>
                    @endforeach
                </ul>
            </div>
            {{-- Resources --}}
            <div>
                ...
                <ul class="space-y-2.5 text-sm">
                    ...
                    <li><a href="#" class="hover:text-green-400 transition">{{ $item }}</a></li>
                    @endforeach
                </ul>
            </div>
            {{-- Support --}}
            ...
            {{-- Contact --}}
            <div>
                <h4 class="text-white font-bold text-sm mb-4">Contact Us</h4>
                <div class="space-y-3 text-sm">
                    ...
                    <p><a href="tel:+2348001234567" class="flex gap-2 hover:text-green-400 transition"><i class="fa-solid fa-phone text-green-500"></i>+234 800 123 4567</a></p>
                    ...
                    <p><a href="https://wa.me/2348001234567" class="flex gap-2 hover:text-green-400 transition"><i class="fa-brands fa-whatsapp text-green-500"></i>+234 800 123 4567</a></p>
                </div>
            </div>
        </div>
        {{-- Newsletter row --}}
        <div class="border-t border-white/10 pt-8 pb-6">
            <div class="grid md:grid-cols-2 gap-6 items-center">
                <div>
                    <h4 class="text-white font-bold text-sm mb-1">Newsletter</h4>
                    <p class="text-xs text-gray-500">Subscribe to our newsletter for latest updates and insights.</p>
                </div>
                <form class="flex gap-2" onsubmit="return subscribeNewsletter(this)">
                    <input type="email" required placeholder="Enter your email" class="flex-1 bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-green-400">
